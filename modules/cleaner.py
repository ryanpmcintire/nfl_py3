...
def assign_stats(df: pd.DataFrame, column: str):
    ...
    homestat = f'h{column}'
    awaystat = f'a{column}'
    df[offensive] = np.where(
        df['at'] == '@', df[awaystat], df[homestat])
    df[defensive] = np.where(
        df['at'] == '@', df[homestat], df[awaystat])
    return df


def parse_home_away_stats(df: pd.DataFrame, spit_col, new_cols):
    data = pd.DataFrame(columns=new_cols)
    data[new_cols] = df[spit_col].str.split(
        r'(?<!-)-', expand=True).astype('int')
    return df.join(data)


def rolling_averages(df, column, key, game_span):
    df[column] = df.groupby(['team'])[key].shift(1).rolling(
...
def determine_home_team(df):
    df['Home_Team'] = np.where(
        df['at'] == '@', df['opponent'], df['team'])
    df['Away_Team'] = np.where(
        df['at'] == '@', df['team'], df['opponent'])
    return df


def determine_home_score(df):
    df['Home_Score'] = np.where(
        df['at'] == '@', df['opp_score'], df['team_score'])
    df['Away_Score'] = np.where(
        df['at'] == '@', df['team_score'], df['opp_score'])
    return df
...
def parse_vegas_line(df):
    ...
    df[['Fav_City', 'Fav_Team', 'Fav_Spread']
              ] = df['Vegas_Line_Close'].str.rsplit(' ', 2, expand=True)
...
def determine_home_spread(df):
    ...
    df['HomeFav'] = (
        df['favorite'] == '000').map({True: 0, False: -1})
    df.loc[df['favorite'] ==
                  df['Home_Team'], 'HomeFav'] = 1
    df['underdog'] = (
        df['favorite'] == df['team']).map({True: 0, False: 1})

    df = df.fillna(0)
    df['Home_Vegas_Spread'] = ([df['Fav_Spread'][i]
                                      if df['HomeFav'][i] == 1 else -1 * df['Fav_Spread'][i] for i in range(len(df))])
    df['Home_Actual_Spread'] = -1 * \
...
def clean(year, week, new_set = pd.DataFrame, game_span = 10):  # type: ignore
    ...
    OPPONENT_ABBRV = REGULAR_SEASON['team'].unique(
    ).tolist()+['ram', 'rai', 'sdg', 'was', 'was']
    OPPONENT_VERBOSE = REGULAR_SEASON['verbose_name'].unique(
    ...
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_score', 'team_score', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_allow', 'opp_score', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_to', 'off_turn_overs', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_fto', 'def_turn_overs', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_pass_eff', 'team_pass_eff', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_pass_def', 'team_pass_def', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_rush_eff', 'team_rush_eff', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_rush_def', 'team_rush_def', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_penYds', 'PenYds', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_penYdsAgg', 'PenYdsAgg', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_third_eff', 'third_eff', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_third_def', 'third_def', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_fourth_eff', 'fourth_eff', game_span)
    REGULAR_SEASON = rolling_averages(REGULAR_SEASON, 'trail_fourth_def', 'fourth_def', game_span)
    ...
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_pass_eff', 'trail_pass_eff')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_pass_def', 'trail_pass_def')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_rush_eff', 'trail_rush_eff')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_rush_def', 'trail_rush_def')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_penYds', 'trail_penYds')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_penYdsAgg', 'trail_penYdsAgg')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_third_eff', 'trail_third_eff')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_third_def', 'trail_third_def')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_fourth_eff', 'trail_fourth_eff')
    REGULAR_SEASON = get_opp_trail(REGULAR_SEASON, 'trail_opp_fourth_def', 'trail_fourth_def')
