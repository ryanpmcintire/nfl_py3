...
from learner_methods import print_eval, get_splits, read_data, plot, read_path, x_cols
import pandas as pd
...
def xgb_learner():
    ...
    xgb_model = xgb.XGBRegressor(n_estimators=1700, objective='reg:squarederror', subsample=0.65, learning_rate=0.0015, tree_method="approx", max_depth=3, booster='gbtree', random_state=88)
    X_train, X_test, y_train, y_test = get_splits(2022, 5)
...
def predict(year, week):
    xgb_model = xgb.XGBRegressor(n_estimators=1500, objective='reg:squarederror', subsample=0.65, learning_rate=0.0015, tree_method="approx", max_depth=3, booster='gbtree', eval_metric='rmse', random_state=88)
