...
- **Opener-graded, probability-rule accuracy (the pool-relevant grade -- picks lock Tuesday against a frozen line):** **54.09%** on **1,537 paired games**, week-blocked 95% interval [51.54%, 56.71%].
...
- **Weak-signal registry:** 766 results recorded -- 760 unresolved_below_power, 6 closed (6 refuted_mechanism, 0 bounded_by_control). An interval crossing zero is never by itself grounds to close a line of work; see `AGENTS.md`.
- **Rotation registry:** 426 declared research families -- 60 open, 1 confirmed/closed/retired, 365 declared for coverage only (no window yet).
- **Prospective challengers:** 31 of 37 registered challengers are actively tracked prospectively (`artifacts/prospective/challengers.json`).
<!-- RESEARCH_STATE:END -->
