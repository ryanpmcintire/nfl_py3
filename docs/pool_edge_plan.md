...
> **Superseded — this reading came from defective machinery.** Four defects in
> the pooling arithmetic were fixed 2026-09-08 (`docs/weak_signal_pooling.md`;
> audit findings B1–B4): the command crashed outright above ~1,030 signals, the
> sign test scored exact ties as baseline wins, `probability_positive` charged
> the zero atom against the candidate, and inverse-variance weighting handed a
> three-game cell 99.997% of the fixed-effect weight. Every one of them pushed
> the same way — toward resolved-looking negatives. Under the fixed code the
> pool reads **-0.0155 accuracy points, 95% [-0.0358, +0.0049],
> `probability_positive` 0.068** on 1,369 signals, with the sign test at
> 576/1,281 informative signals (208 ties excluded, p=0.00034). Still
> unresolved, still closes nothing. Re-run the command.
