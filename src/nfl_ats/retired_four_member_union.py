...
def retired_union_sides(primary: pd.DataFrame) -> pd.Series:
    ...
    zone = (
        ...
        .abs()
        .between(SPREAD_GAP_LOWER_BOUND, SPREAD_GAP_UPPER_BOUND)
    )
    flip = primary["composed_overlay_flip"].astype(bool) | zone
    model = primary["model_pick_side"].astype(str)
    return model.where(~flip, model.map({"HOME": "AWAY", "AWAY": "HOME"}))


def record_retired_four_member_union_decisions(
    artifacts_root: Path,
    data_root: Path,
    *,
    ...
    former_sides = retired_union_sides(fresh)
...
__all__ = ["record_retired_four_member_union_decisions"]
