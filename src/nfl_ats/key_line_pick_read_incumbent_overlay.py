...
def smooth_card(card: pd.DataFrame, sidecar: dict[str, Any]) -> pd.DataFrame:
    ...
    if not isinstance(games, list):
        raise DataContractError(f"{KEY_LINE_PICK_READ_FILENAME} carries no per-game reads")
    by_game: dict[str, float] = {}
    for row in games:
        if not isinstance(row, dict):
            continue
        value = row.get("home_cover_probability_smooth")
        if value is None:
            continue
    ...
    if missing:
        raise DataContractError(
            f"{KEY_LINE_PICK_READ_FILENAME} lacks the smooth read for: {', '.join(missing)}"
        )
    result = card.copy()
...
def record_key_line_pick_read_incumbent_challenger_decisions(
    artifacts_root: Path,
    data_root: Path,
    *,
    ...
    if sidecar is None or not sidecar.get("served"):
        raise DataContractError(
            f"Active forecast {forecast.name} carries no served {KEY_LINE_PICK_READ_FILENAME}; "
            "a card produced without the key-line pick read has no paired read to record"
        )
    ...
    paired = smooth_card(card, sidecar)
