...
def _week_card(
    ...
    *,
    ...
    predicted = model.predict(target, probability_method="gaussian_median")
    card = target.copy()
...
# ---------------------------------------------------------------------------
# 1. apply_gaussian_mean_mapping_incumbent_overlay
# ---------------------------------------------------------------------------
...
def test_overlay_changes_every_probability_and_only_that_column(model_frame: pd.DataFrame) -> None:
    ...
    expected = models["market_residual"].predict(target, probability_method="gaussian")
    np.testing.assert_allclose(mapped, expected["home_cover_probability"], rtol=0, atol=1e-12)
...
def test_overlay_disabled_is_a_no_op(model_frame: pd.DataFrame) -> None:
    card = _week_card(model_frame)
    result = apply_gaussian_mean_mapping_incumbent_overlay(card, model_frame, enabled=False)
...
def test_overlay_requires_its_prediction_columns(model_frame: pd.DataFrame) -> None:
    with pytest.raises(DataContractError, match="overlay columns"):
        apply_gaussian_mean_mapping_incumbent_overlay(
            pd.DataFrame({"game_id": ["G1"]}), model_frame
        )
...
def test_overlay_refuses_a_mean_card(
    ...
    model = margin_models["market_residual"]
    predicted_mean = model.predict(target, probability_method="gaussian")
    mean_card = target.copy()
    mean_card["home_cover_probability"] = predicted_mean["home_cover_probability"].to_numpy()
...
def test_disclosure_note_is_empty_when_disabled(model_frame: pd.DataFrame) -> None:
    card = _week_card(model_frame)
    disabled = apply_gaussian_mean_mapping_incumbent_overlay(card, model_frame, enabled=False)
...
# ---------------------------------------------------------------------------
# 3. record_gaussian_mean_mapping_incumbent_challenger_decisions
# ---------------------------------------------------------------------------
...
def test_record_challenger_decisions_records_the_mapping_arm(
    ...
    result = record_gaussian_mean_mapping_incumbent_challenger_decisions(
    ...
    expected = apply_gaussian_mean_mapping_incumbent_overlay(
        _week_card(model_frame),
        model_frame,
        feature_profile=_FEATURE_PROFILE,
        min_train_games=_MIN_TRAIN_GAMES,
        ridge_alpha=_RIDGE_ALPHA,
    ...
    expected_sides = dict(
        zip(
            expected["game_id"],
            np.where(expected["home_cover_probability"].ge(0.5), "HOME", "AWAY"),
            strict=True,
        )
    )
    assert ledger["pick_side"].tolist() == ledger["game_id"].map(expected_sides).tolist()
    ...
    again = record_gaussian_mean_mapping_incumbent_challenger_decisions(
        artifacts, data_root, now=now
    )
