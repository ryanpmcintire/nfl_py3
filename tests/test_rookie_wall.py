...
def test_top50_pick_lookup_flags_only_resolved_top50_picks() -> None:
    ...
    combine = pd.DataFrame(
        [
            _combine_row("pfr1", 1.0, 2020.0),
            _combine_row("pfr2", TOP50_DRAFT_OVR_MAX + 1.0, 2020.0),
            _combine_row("pfr3", 5.0, 2020.0),
            _combine_row("pfr4", None, 2020.0),
            _combine_row(None, 3.0, 2020.0),
        ]
    )
    ...
    assert diagnostics["n_with_draft_ovr"] == 4
    assert diagnostics["n_with_draft_ovr_and_pfr_id"] == 3
    assert diagnostics["n_joined_to_gsis"] == 2
...
def test_top50_pick_lookup_keeps_earliest_draft_year_on_duplicate_pfr_id() -> None:
    ...
    combine = pd.DataFrame(
        [
            _combine_row("pfr1", 60.0, 2020.0),
            _combine_row("pfr1", 2.0, 2019.0),
        ]
    )
...
def test_wall_candidates_separates_rookie_top50_and_veteran_populations() -> None:
    ...
    add_player("00-rookie-top50", career_age=0, is_top50=True, workload=0.80)
    add_player("00-rookie-late", career_age=0, is_top50=False, workload=0.80)
    add_player("00-vet-heavy", career_age=5, is_top50=False, workload=0.80)
    add_player("00-vet-light", career_age=5, is_top50=False, workload=0.20)
    add_player("00-vet-young", career_age=1, is_top50=False, workload=0.80)
...
def test_trailing_dependence_feature_excludes_the_current_weeks_own_value() -> None:
    ...
    assert week3_before == pytest.approx(week3_after)
...
def test_team_season_split_half_pairs_odd_and_even_weeks_per_team_season() -> None:
    ...
    assert pivot.loc[("KC", 2020), "odd"] == pytest.approx(1.0)
    assert pivot.loc[("KC", 2020), "even"] == pytest.approx(0.0)
