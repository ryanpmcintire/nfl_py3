...
from nfl_ats.smooth_cdf_mapping_overlay import (
    CHALLENGER_ID,
    SmoothCdfMappingFlip,
    SmoothCdfMappingResult,
    apply_smooth_cdf_mapping_overlay,
    overlay_disclosure_note,
    record_smooth_cdf_mapping_challenger_decisions,
)
...
_SEASON = 2020
_WEEK = 4
_FEATURE_TABLE_NAME = "smooth_cdf_test_features.parquet"
...
# ---------------------------------------------------------------------------
# 1. apply_smooth_cdf_mapping_overlay
# ---------------------------------------------------------------------------
...
def test_overlay_changes_every_probability_and_only_that_column(model_frame: pd.DataFrame) -> None:
    ...
    # The Gaussian read differs from the raw ECDF for at least one game, and
    # never by a wild amount (a sanity bound, not a calibration claim).
...
def test_overlay_disabled_is_a_no_op(model_frame: pd.DataFrame) -> None:
    card = _week_card(model_frame)
    result = apply_smooth_cdf_mapping_overlay(card, model_frame, enabled=False)
...
def test_overlay_requires_its_prediction_columns(model_frame: pd.DataFrame) -> None:
    with pytest.raises(DataContractError, match="overlay columns"):
        apply_smooth_cdf_mapping_overlay(pd.DataFrame({"game_id": ["G1"]}), model_frame)
...
def test_overlay_refuses_a_drifted_probability(model_frame: pd.DataFrame) -> None:
    ...
    with pytest.raises(DataContractError, match="drifted"):
        apply_smooth_cdf_mapping_overlay(
            card,
            model_frame,
            regressor="ridge",
...
def test_disclosure_note_is_empty_when_disabled(model_frame: pd.DataFrame) -> None:
    card = _week_card(model_frame)
    disabled = apply_smooth_cdf_mapping_overlay(card, model_frame, enabled=False)
    ...
    result = SmoothCdfMappingResult(
        ...
        flips=(
            SmoothCdfMappingFlip(
                ...
                from_side="HOME",
                to_side="AWAY",
                ecdf_probability=0.51,
                gaussian_probability=0.49,
            ),
        ),
        enabled=True,
    )
...
# ---------------------------------------------------------------------------
# 3. record_smooth_cdf_mapping_challenger_decisions
# ---------------------------------------------------------------------------
...
def test_record_challenger_decisions_records_the_mapping_arm(
    ...
    result = record_smooth_cdf_mapping_challenger_decisions(artifacts, data_root, now=now)
    ...
    again = record_smooth_cdf_mapping_challenger_decisions(artifacts, data_root, now=now)
