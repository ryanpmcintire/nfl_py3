"""``weak_stack_deadline_drag`` prospective challenger recording.
...
alongside a real chronological fit regression test: this pins the
...
import nfl_ats.deadline_drag_challenger as challenger_module
from nfl_ats.data import DataContractError
from nfl_ats.deadline_drag_challenger import (
...
def _write_active_model_and_card(
    ...
    pd.DataFrame({"game_id": ["placeholder"], "kickoff": ["2020-01-01T00:00:00Z"]}).to_parquet(
        features_path
    )
...
def test_record_challenger_decisions_records_every_game_and_flags_diffs(
    ...
    result = record_deadline_drag_challenger_decisions(artifacts, tmp_path, now=now)
...
def test_both_arms_are_frozen_on_retry(tmp_path, monkeypatch):
    ...
    now = KICKOFF - pd.Timedelta(days=3)
    record_deadline_drag_challenger_decisions(artifacts, tmp_path, now=now)
...
def test_missing_transaction_source_skips_cleanly(tmp_path, monkeypatch):
    artifacts = tmp_path / "artifacts"
    _write_registry(artifacts)
    _write_active_model_and_card(artifacts, tmp_path)
    result = record_deadline_drag_challenger_decisions(
        artifacts, tmp_path, now=KICKOFF - pd.Timedelta(days=3)
    )
...
@pytest.mark.parametrize("publication", ["2026-11-03T13:00:00Z", "2026-11-09T00:00:00Z"])
def test_transactions_after_publication_or_pool_cutoff_are_invisible(publication):
    cutoff = pd.Timestamp("2026-11-08T21:00:00Z")
    transactions = pd.DataFrame(
        {
            "url_year": 2026,
            "url_month": 10,
            "slug": ["visible", "after_publication", "at_cutoff", "after_cutoff", "unknown"],
            "observed_at_utc": [
    ...
    got = challenger_module.visible_transactions(
        transactions, cutoff, pd.Timestamp(publication), pd.Timestamp(publication), 2026
    )
    assert got.slug.tolist() == (
        ["visible"] if pd.Timestamp(publication) < cutoff else ["visible", "after_publication"]
    )


def test_current_wire_requires_predecision_capture_and_ignores_lastmod():
    cutoff = pd.Timestamp("2026-11-08T21:00:00Z")
    transactions = pd.DataFrame(
        {
            "url_year": [2025, 2026],
            "url_month": [10, 10],
            "lastmod_contaminated": ["2000-01-01", "2000-01-01"],
        }
    )
    got = challenger_module.visible_transactions(
        transactions, cutoff, cutoff + pd.Timedelta(days=1), cutoff, 2026
    )
    assert got.url_year.tolist() == [2025]


def test_future_transaction_snapshot_refused(tmp_path):
    path = tmp_path / "raw/pfr_transactions/20261109T000000Z/index.parquet"
    path.parent.mkdir(parents=True)
    path.touch()
    with pytest.raises(DataContractError, match="future-dated"):
        challenger_module.build_stacked_features(
            pd.DataFrame(), tmp_path, pd.Timestamp("2026-11-03T13:00:00Z"), pd.DataFrame()
        )


def test_registered_profile_fits_real_chronological_model(model_frame):
    from nfl_ats.constants import FEATURE_SETS
    from nfl_ats.margin import MARGIN_FEATURE_PROFILES, margin_feature_columns

    assert CANDIDATE_FEATURE_PROFILE in MARGIN_FEATURE_PROFILES
    columns = margin_feature_columns("market_residual", CANDIDATE_FEATURE_PROFILE)
    assert columns == FEATURE_SETS["full_weak_stack"] + ("deadline_integration_drag_flag",)
    ...
    target, models = challenger_module.fit_margin_models_for_week(frame, **kwargs)
    before = models["market_residual"].predict(target, probability_method="gaussian")
    frame.loc[frame.season.ge(2020) & frame.week.ge(4), ["result", "ats_margin"]] = 99999.0
    target, models = challenger_module.fit_margin_models_for_week(frame, **kwargs)
    after = models["market_residual"].predict(target, probability_method="gaussian")
    pd.testing.assert_frame_equal(before, after)
    assert len(before) > 0


def test_dashboard_display_name_is_plain_english():
    from nfl_ats.dashboard.findings_content import CHALLENGER_DISPLAY_NAMES

    assert CHALLENGER_DISPLAY_NAMES[CHALLENGER_ID] == "Adjustment after a trade"


def test_real_feature_builder_hides_late_transactions(tmp_path, monkeypatch):
    cutoff = pd.Timestamp("2026-11-08T21:00:00Z")
    path = tmp_path / "raw/pfr_transactions/20261103T120000Z/index.parquet"
    path.parent.mkdir(parents=True)
    transactions = pd.DataFrame(
        {
            "slug": ["eagles-acquire-fake-player-from-old"],
            "transaction_relevant": [True],
            "url_year": [2026],
            "url_month": [10],
            "observed_at_utc": [cutoff - pd.Timedelta(seconds=1)],
        }
    )
    transactions.to_parquet(path)
    ...
    monkeypatch.setattr(challenger_module, "default_schedule", lambda root: schedule)
    monkeypatch.setattr(challenger_module, "default_snap_counts", lambda root: snaps)
    now = cutoff + pd.Timedelta(hours=1)
    got = challenger_module.build_stacked_features(schedule, tmp_path, now, schedule)
    assert got.deadline_integration_drag_flag.tolist() == [-1.0]
    transactions["observed_at_utc"] = cutoff
    transactions.to_parquet(path)
    got = challenger_module.build_stacked_features(schedule, tmp_path, now, schedule)
    assert got.deadline_integration_drag_flag.tolist() == [0.0]
    transactions["observed_at_utc"] = now + pd.Timedelta(seconds=1)
    transactions.to_parquet(path)
    got = challenger_module.build_stacked_features(schedule, tmp_path, now, schedule)
    assert got.deadline_integration_drag_flag.tolist() == [0.0]
