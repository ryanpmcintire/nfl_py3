"""Leakage regression + wiring tests for the ``weak_stack_graph_def_ypp``
candidate profile's one new column
(docs/graph_team_stat_def_ypp_on_production.md), per AGENTS.md's "a leakage
regression test for every new feature family" rule. Mirrors
``tests/test_graph_team_stat_production_feature.py`` (the off_sack_rate
sibling) exactly, substituting the ``def_yards_per_play`` team_stat column.
...
from nfl_ats.graph_team_stat_def_ypp_production_feature import (
...
def test_weak_stack_graph_def_ypp_profile_is_registered_and_disjoint_from_production_sets() -> None:
    assert "weak_stack_graph_def_ypp" in MARGIN_FEATURE_PROFILES
    assert set(GRAPH_TEAM_STAT_DEF_YARDS_PER_PLAY_FEATURE_COLUMNS).isdisjoint(MODEL_FEATURE_COLUMNS)
    for name in ("football_weak_stack", "full_weak_stack", "football", "full"):
        assert set(FEATURE_SETS[name]).isdisjoint(
            GRAPH_TEAM_STAT_DEF_YARDS_PER_PLAY_FEATURE_COLUMNS
        )
    # weak_stack_graph_def_ypp = weak_stack plus exactly the one new column --
    # never used by the active model.
    graph_def_ypp_columns = set(
        margin_feature_columns("market_residual", "weak_stack_graph_def_ypp")
    )
    weak_stack_columns = set(margin_feature_columns("market_residual", "weak_stack"))
    assert graph_def_ypp_columns - weak_stack_columns == set(
        GRAPH_TEAM_STAT_DEF_YARDS_PER_PLAY_FEATURE_COLUMNS
    )
    assert len(GRAPH_TEAM_STAT_DEF_YARDS_PER_PLAY_FEATURE_COLUMNS) == 1
            ...
            rows.append(
                {
                    ...
                    # Vary the raw def_yards_per_play differential so the graph
                    # has a real signal to propagate rather than an all-zero
                    # matrix.
                    "home_def_yards_per_play": 5.2 + 0.05 * index,
                    "away_def_yards_per_play": 5.6 - 0.05 * index,
                }
            )
...
def test_derive_graph_def_ypp_feature_is_nan_before_min_games_and_rated_after() -> None:
    schedule = _schedule(n_weeks=5)
    derived = derive_graph_def_ypp_feature(schedule).set_index("game_id")

    early = derived.loc[derived.index.str.startswith("2021_01_")]
    assert early[GRAPH_DEF_YPP_COLUMN].isna().all()

    late = derived.loc[derived.index.str.startswith("2021_05_")]
    assert late[GRAPH_DEF_YPP_COLUMN].notna().all()


def test_attach_graph_def_ypp_feature_is_purely_additive() -> None:
    schedule = _schedule(n_weeks=5)
    base = schedule.copy()
    base["some_pre_existing_column"] = range(len(base))

    widened = attach_graph_def_ypp_feature(base)
    ...
    assert new_columns == set(GRAPH_TEAM_STAT_DEF_YARDS_PER_PLAY_FEATURE_COLUMNS)


def test_attach_graph_def_ypp_feature_refuses_a_collision() -> None:
    schedule = _schedule(n_weeks=1)
    schedule[GRAPH_DEF_YPP_COLUMN] = 0.0
    with pytest.raises(DataContractError):
        attach_graph_def_ypp_feature(schedule)


def test_attach_graph_def_ypp_feature_is_leak_safe_across_a_future_week() -> None:
    ...
    perturbing week 6's own result and raw def_yards_per_play columns (a
    ...
    baseline = attach_graph_def_ypp_feature(schedule).set_index("game_id")

    mutated = schedule.copy()
    perturbed_week = mutated["game_id"].str.startswith("2021_06_")
    mutated.loc[perturbed_week, "result"] = 45.0
    mutated.loc[perturbed_week, "home_def_yards_per_play"] = 9.0
    mutated.loc[perturbed_week, "away_def_yards_per_play"] = 2.0
    changed = attach_graph_def_ypp_feature(mutated).set_index("game_id")

    # Only the DERIVED graph column is compared here -- week 6's own raw
    # pass-through columns (result, home/away_def_yards_per_play) differ from
    ...
    pd.testing.assert_series_equal(
        changed.loc[unaffected, GRAPH_DEF_YPP_COLUMN],
        baseline.loc[unaffected, GRAPH_DEF_YPP_COLUMN],
        check_exact=True,
    )
    ...
    assert not changed.loc[following_week, GRAPH_DEF_YPP_COLUMN].equals(
        baseline.loc[following_week, GRAPH_DEF_YPP_COLUMN]
    )
