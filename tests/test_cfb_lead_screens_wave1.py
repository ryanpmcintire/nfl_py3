...
def test_post_bye_signed_column_matches_hand_computed_cases() -> None:
    ...
    assert attached.loc[101, POST_BYE_COLUMN] == 1.0
    assert np.isnan(attached.loc[102, POST_BYE_COLUMN])
    assert attached.loc[103, POST_BYE_COLUMN] == -1.0
    assert attached.loc[104, POST_BYE_COLUMN] == 0.0
    assert attached.loc[105, POST_BYE_COLUMN] == 0.0
...
def test_rivalry_home_dog_requires_both_rivalry_and_underdog() -> None:
    ...
    assert attached.loc[201, RIVALRY_COLUMN] == 1.0
    assert attached.loc[202, RIVALRY_COLUMN] == 0.0
    assert attached.loc[203, RIVALRY_COLUMN] == 0.0
    assert attached.loc[204, RIVALRY_COLUMN] == 0.0
...
def test_altitude_cold_home_requires_home_team_and_october_on() -> None:
    ...
    assert attached.loc[301, ALTITUDE_COLUMN] == 0.0
    assert attached.loc[302, ALTITUDE_COLUMN] == 1.0
    assert attached.loc[303, ALTITUDE_COLUMN] == 0.0
    assert attached.loc[304, ALTITUDE_COLUMN] == 1.0
    assert attached.loc[305, ALTITUDE_COLUMN] == 0.0
