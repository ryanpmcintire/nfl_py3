...
def capture(
    ...
    *,
    ...
    if any(year < 2009 or year > 2026 for year in seasons):
        raise ValueError("Historical source scope is 2009-2026")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Capture dated Wikipedia staff revisions for HC/OC/DC identity; source "
            "acquisition only, never an ATS experiment. One historical revision per "
            "team/season/cutoff is a conservative observation, not a claim to have found "
            "the first announcement or every intervening change. Successful responses "
            "are reused across immutable snapshots on subsequent runs."
        )
    )
