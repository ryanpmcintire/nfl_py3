...
def block_bootstrap_two_group(
    ...
    *,
    ...
    Generalizes ``penalty_discipline_interval.py::block_bootstrap_quartile_gap``
    from two extreme quartiles to an arbitrary boolean subset vs. its
    complement over the WHOLE population, jointly resampling both arms from
    the same drawn set of blocks each draw. Draws where a resampled set has
    zero rows in either arm are dropped (rare, only possible for very thin
    subsets) and reported as ``dropped_draws``.
    """
