"""Standalone weekly recorder for the ``turnover_luck_rebound_tilt_overlay`` challenger.
...
``nfl_ats.turnover_luck_rebound_tilt_overlay.record_turnover_luck_rebound_tilt_challenger_decisions``
...
Usage (from the repo root, per AGENTS.md environment conventions)::
    ...
    .\\.tools\\uv.exe run --no-sync python scripts/record_turnover_luck_rebound_challenger.py
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = record_turnover_luck_rebound_tilt_challenger_decisions(
        args.artifacts_root, args.data_root, now=now
    )
