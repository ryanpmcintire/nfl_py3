"""Third-down mean-reversion fade, stacked on PRODUCTION: does it beat what is
actually played, not a bare market baseline?

Predeclared in ``docs/redzone_reversion_on_production.md`` BEFORE this script
was pointed at an outcome column. Read that document first -- it declares the
candidate column, both deliberate deviations from the registered cell, the two
arms, the rotation family/window, the null, the positive control, and the
recording rules. Copied from
``scripts/graph_team_stat_def_yards_per_play_on_production.py`` (the single-arm
on-production runner) with the profile, column, family, artifact directory and
predeclaration path substituted.

The construct: ``redzone_reversion_c3_third_down_over_fade``, the leading cell
of the frozen battery in ``docs/redzone_reversion_screen.md`` (+0.3665 accuracy
points, week-blocked 95% [-0.2587, +0.9990], ``probability_positive`` 0.8719,
reliability 0.407) -- both the highest lean and the highest split-half
reliability in that battery. Its mirror cell,
``redzone_reversion_c4_third_down_under_rebound``, sits at P+ 0.0989: the
opposite tail, which is the shape a reversion mechanism predicts and a pure
quality proxy does not.
...
* ``candidate`` -- ``weak_stack_redzone_third_down``: the SAME production
  feature set (90 columns) plus exactly one new column,
...
measures the fade column's marginal contribution on top of everything the
...
* ``positive-control`` -- the candidate's one new column is replaced by the
  ...
  "no effect" reading from it would mean nothing. If this check does not show
  a large obvious effect the screen is NOT read.
* ``screen``           -- the real look. Spends the family's assigned
  rotation window (``redzone_reversion_on_production``); run once, after both
  checks above pass.
...
regardless of sign -- the family here is recorded ``unresolved_below_power``.
"""
...
from nfl_ats.redzone_reversion_production_feature import (  # noqa: E402
    REDZONE_THIRD_DOWN_OVER_FADE_COLUMN,
)

BASELINE_PROFILE: MarginFeatureProfile = "weak_stack"
CANDIDATE_PROFILE: MarginFeatureProfile = "weak_stack_redzone_third_down"
REGRESSOR = "ridge"
RIDGE_ALPHA = 10.0
...
def run_window(
    ...
    *,
    ...
    if leak_treatment:
        candidate_source[REDZONE_THIRD_DOWN_OVER_FADE_COLUMN] = pd.to_numeric(
    ...
    for (_season, _week), group in window.groupby(["season", "week"], sort=True):
        ...
        candidate_scoring = (
            ...
            if not leak_treatment
                ...
                **{
                    REDZONE_THIRD_DOWN_OVER_FADE_COLUMN: pd.to_numeric(
                        group["ats_margin"], errors="coerce"
                    )
                }
            )
        )
...
def permuted_margins(
    ...
    ``docs/redzone_reversion_on_production.md`` section 6 for why this null is
    not centred on zero by design: it preserves each week's realized home-cover
...
def column_distribution(features: pd.DataFrame, seasons: tuple[int, ...]) -> dict[str, Any]:
    """The candidate column's coverage and value distribution INSIDE the
    assigned window -- disclosed in section 6 as a power caveat, reported in
    section 7 as a measured fact."""

    frame = regular_season_rows(features)
    frame = frame.loc[frame["season"].astype(int).isin(seasons)]
    frame = frame.loc[frame["result"].notna()]
    values = pd.to_numeric(frame[REDZONE_THIRD_DOWN_OVER_FADE_COLUMN], errors="coerce")
    covered = values.dropna()
    return {
        "n_games": len(frame),
        "coverage": float(values.notna().mean()) if len(values) else float("nan"),
        "n_covered": len(covered),
        "share_minus_one": float((covered == -1.0).mean()) if len(covered) else float("nan"),
        "share_zero": float((covered == 0.0).mean()) if len(covered) else float("nan"),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "the redzone_reversion_on_production family's assigned window)",
    )
    ...
    distribution = column_distribution(features, seasons)
    print(
        f"in-window column: coverage {distribution['coverage']:.3%} of "
        f"{distribution['n_games']} games; -1 {distribution['share_minus_one']:.3%} / "
        f"0 {distribution['share_zero']:.3%} / +1 {distribution['share_plus_one']:.3%}"
    )
    ...
    configuration = {
        ...
        "candidate_column": REDZONE_THIRD_DOWN_OVER_FADE_COLUMN,
        "regressor": REGRESSOR,
        "ridge_alpha": RIDGE_ALPHA,
        "bootstrap_samples": args.bootstrap_samples,
        "seed": args.seed,
        "permutations": args.permutations,
        "predeclaration": "docs/redzone_reversion_on_production.md",
    ...
    output_dir = (
        REPO_ROOT / "artifacts" / (args.out or "redzone_reversion_on_production") / timestamp
    )
    write_experiment_artifact(
        output_dir,
        "results.json",
        payload,
        command="redzone-reversion-on-production",
        metrics={"mode": args.mode, "status": result.get("status", "unknown")},
        notes=(
            "Third-down mean-reversion fade column (redzone_reversion_screen C3, "
            "rebuilt with an expanding strictly-prior threshold) vs PRODUCTION "
            "weak_stack (not a bare market baseline); see "
            "docs/redzone_reversion_on_production.md."
        ),
    )
    ...
    if args.mode == "null" and result.get("status") == "scored":
        ...
        print()
        print(
            f"candidate (weak_stack_redzone_third_down) minus baseline (weak_stack), {args.mode}:"
        )
        if pair is not None:
            low, high = pair["week_blocked_ci95"]
            s_low, s_high = pair["season_blocked_ci95"]
            print(
            ...
            print(
                f"baseline accuracy {pair['baseline_accuracy'] * 100:.3f}%  candidate "
                f"{pair['candidate_accuracy'] * 100:.3f}%  season 95% CI "
                f"[{s_low * 100:+.3f}, {s_high * 100:+.3f}] P+ "
                f"{pair['season_blocked_probability_positive']:.3f}"
            )
        print(f"home-pick rate: {result['home_pick_rate']}")
        print(
