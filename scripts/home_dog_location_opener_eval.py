"""Frozen lane Q home-underdog location experiment; see docs/home_dog_location.md.
...
from nfl_ats.home_dog_location import HOME_DOG_HINGE_COLUMNS, attach_home_dog_location
...
FAMILY = "mod18_home_dog_location_v1"
ARMS = {"Q1": "weak_stack_home_dog_points", "Q2": "weak_stack_home_dog_hinge_7"}
DISCOUNT = (
    "Mined archive; inherited active-model, lane H/K/L spread diagnosis (which motivated "
    "this feature) and overlay-subset selection; two correlated arms; descriptive reuse, "
    "not independent confirmation."
)
SUMMARIES = {
    "Q1": (
        "When the home team is the underdog, the model learns from past seasons how much "
        "to give back to the home side for each point it is behind, so home underdogs get a "
        "fairer shake without any hand-set rule."
    ),
    "Q2": (
        "When the home team is the underdog, the model learns from past seasons how much "
        "to give back to the home side for each point it is behind, and it may add a little "
        "more once the home team is more than a touchdown behind."
    ),
}
...
def cli(out: Path, *args: str) -> str:
    ...
    if arguments[:2] == ["weak-signals", "record"] and "--plain-summary" not in arguments:
        ...
        arguments.extend(["--plain-summary", SUMMARIES["Q2" if "_Q2" in name else "Q1"]])
...
def record(out: Path, name: str, m: dict, units: str, start: int, end: int, source: Path) -> None:
    ...
    cli(
        ...
        FAMILY,
        "--description",
        f"Home-underdog location feature {name}, positive favours the candidate",
        "--source",
        str(source),
        "--classification",
...
def per_point_coefficients(model: MarginModel) -> dict[str, float]:
    """Fitted ridge weight per point of home underdog, unscaled."""
    ...
    out = {}
    for column in HOME_DOG_HINGE_COLUMNS:
...
def build(out: Path, archive: pd.DataFrame) -> None:
    ...
    features = attach_home_dog_location(regular_season_rows(pd.read_parquet(common.FEATURES)))
    ...
    for (season, week), group in targets.groupby(["season", "week"], sort=True):
        ...
        if len(training) < 500:
            continue
        scoring = attach_home_dog_location(scoring)
        ...
        for name, profile in (("base", "weak_stack"), *ARMS.items()):
    ...
    common.table(pd.concat(batches, ignore_index=True), out / "stream.parquet")
    common.table(pd.DataFrame(weights), out / "coefficients.parquet")
    write_stamped_artifact(
        {
            "active": active,
            "archive": str(common.ARCHIVE),
            "predeclaration_sha256": sha256_file(common.REPO / "docs/home_dog_location.md"),
            "profiles": ARMS,
        },
        out / "inputs.json",
    )
...
def losses(out: Path, archive: pd.DataFrame, stream: pd.DataFrame) -> None:
    ...
    for arm, column in {"incumbent": "home_cover_probability_at_open", **ARMS}.items():
        column = f"p_{arm}" if arm in ARMS else column
        f = valid.copy()
    ...
    replication = {}
    for label, block in (("2011-2017", early.loc[early.season.le(2017)]), ("2018-2019", early)):
        block = block.loc[block.season.ge(2018)] if label == "2018-2019" else block
        block = block.reset_index(drop=True)
        truth_early = block.home_cover.eq(1).to_numpy()
        base_early = block.p_base.ge(0.5).to_numpy()
        for arm in ARMS:
            diff = (block[f"p_{arm}"].ge(0.5).to_numpy() == truth_early).astype(float) - (
                base_early == truth_early
            ).astype(float)
            replication[f"{arm}_{label}"] = metric(block, 100 * diff)
...
def diagnosis(out: Path, archive: pd.DataFrame, stream: pd.DataFrame) -> None:
    """Lane L's Diagnosis D on the incumbent AND each candidate's point forecast."""
    frame = archive.merge(
...
def composition(out: Path, archive: pd.DataFrame, stream: pd.DataFrame) -> None:
    ...
    for arm, profile in ARMS.items():
        ...
        metadata.update(
            ...
            active_model_id=f"research_laneQ_{arm}",
            feature_profile=profile,
            research_scope=(
        ...
        metadata["active_model_config"] = {
            **metadata["active_model_config"],
            "feature_profile": profile,
            "model_id": f"research_laneQ_{arm}",
        }
...
def week1(out: Path, stream: pd.DataFrame) -> None:
    active = json.loads((common.REPO / "artifacts/active_ats_model.json").read_text())
    ...
    week = week.merge(
        current[["game_id", "home_team", "away_team", "spread_line", "home_cover_probability"]],
        on="game_id",
        validate="one_to_one",
        suffixes=("", "_forecast"),
    )
    ...
    for arm, profile in ARMS.items():
        ...
        week[f"cli_max_probability_difference_{arm}"] = float(
            (scratch.home_cover_probability - harness[f"p_{arm}"]).abs().max()
        )
    write_stamped_artifact(
        {
            ...
            "cli_max_probability_difference": {
                arm: float(week[f"cli_max_probability_difference_{arm}"].iloc[0]) for arm in ARMS
            },
            "changes": {
                arm: week.loc[
                    week[f"changed_{arm}"],
