...
SEED = 20260923
SAMPLES = 4000
CFB_TRAIN_SEASONS = (2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2021, 2022, 2023, 2024, 2025)
...
CFB_EXCLUDED_SEASONS_NOTE = (
    "2020 excluded: source archive documents zero opener rows for any book that season "
    "(cfbd_provider_sparse era), not a fetchable gap"
)
...
def load_nfl_population() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    population = population.dropna(subset=[*FEATURES, "open_move", "home_covered"]).reset_index(
        drop=True
    )
...
def paired_mae_effect(
    df: pd.DataFrame, candidate_abs_err: str, baseline_abs_err: str
) -> dict[str, float]:
    def metric_fn(sub: pd.DataFrame) -> dict[str, float]:
...
def split_half_reliability(cfb: pd.DataFrame, nfl: pd.DataFrame) -> float:
    ...
    beta_a = fit_logit_beta(
        design(half_a, FEATURES, means_a, stds_a), half_a["home_covered"].to_numpy()
    )
    means_b, stds_b = standardize(half_b, FEATURES)
    beta_b = fit_logit_beta(
        design(half_b, FEATURES, means_b, stds_b), half_b["home_covered"].to_numpy()
    )
...
def main() -> None:
    ...
    beta_logit = fit_logit_beta(
        design(cfb, FEATURES, means_cfb, stds_cfb), cfb["home_covered"].to_numpy()
    )
    ...
    means_full, stds_full = _standardisers(nfl)
    beta_full = _fit_logit(
        _design(nfl, means_full, stds_full), nfl["home_covered"].to_numpy(), FIT_RIDGE
    )
    base_is_p = _predict(nfl, beta_full, means_full, stds_full)
    record_look("in_sample_fit_base_four_term")
    base_in_sample = {
        "betas": {
            "intercept": float(beta_full[0]),
            **{name: float(beta_full[idx + 1]) for idx, name in enumerate(FIT_FEATURES)},
        },
        "metrics": prob_metrics(base_is_p, nfl["home_covered"].to_numpy()),
        "n": len(nfl),
    }

    plus_features = [*FIT_FEATURES, "cfb_transfer_logit"]
    nfl["plus_oos_p"], plus_fold_betas = loso_logit(
        nfl, plus_features, "home_covered", "base_plus_cfb_transfer"
    )
    nfl["nfl_only_oos_p"], nfl_only_fold_betas = loso_logit(
        nfl, FEATURES, "home_covered", "nfl_only_twin"
    )
    plus_in_sample = in_sample_fit(nfl, plus_features, "home_covered", "base_plus_cfb_transfer")
    nfl_only_in_sample = in_sample_fit(nfl, FEATURES, "home_covered", "nfl_only_twin")
    ...
    nfl["nfl_only_correct"] = (
        nfl["nfl_only_pick"].astype(float).eq(nfl["home_covered"]).astype(float)
    )
    ...
    cell_cfb_direct_vs_base = {
        ...
        "candidate_metrics": prob_metrics(
            nfl["cfb_pred_p"].to_numpy(), nfl["home_covered"].to_numpy()
        ),
        "baseline_metrics": prob_metrics(
    ...
    cell_plus_vs_base = {
        ...
        "candidate_metrics": plus_oos_metrics,
        "baseline_metrics": prob_metrics(
            nfl["base_oos_p"].to_numpy(), nfl["home_covered"].to_numpy()
        ),
        "calibration": calibration_table(nfl, "plus_oos_p", "home_covered"),
        "loso_fold_betas": plus_fold_betas,
        "in_sample_fit": plus_in_sample,
        "in_sample_vs_oos_gap": {
            "accuracy": plus_in_sample["metrics"]["accuracy"] - plus_oos_metrics["accuracy"],
            "brier": plus_in_sample["metrics"]["brier"] - plus_oos_metrics["brier"],
            "log_loss": plus_in_sample["metrics"]["log_loss"] - plus_oos_metrics["log_loss"],
        },
    }
    ...
    cell_nflonly_vs_base = {
        ...
        "candidate_metrics": prob_metrics(
            nfl["nfl_only_oos_p"].to_numpy(), nfl["home_covered"].to_numpy()
        ),
        "baseline_metrics": prob_metrics(
            nfl["base_oos_p"].to_numpy(), nfl["home_covered"].to_numpy()
        ),
    ...
    nfl_only_oos_metrics = prob_metrics(
        nfl["nfl_only_oos_p"].to_numpy(), nfl["home_covered"].to_numpy()
    )
    cell_nflonly_vs_cfbdirect = {
        ...
        "nfl_only_loso_fold_betas": nfl_only_fold_betas,
        "nfl_only_in_sample_fit": nfl_only_in_sample,
        "nfl_only_in_sample_vs_oos_gap": {
            "accuracy": nfl_only_in_sample["metrics"]["accuracy"]
            - nfl_only_oos_metrics["accuracy"],
            "brier": nfl_only_in_sample["metrics"]["brier"] - nfl_only_oos_metrics["brier"],
            "log_loss": nfl_only_in_sample["metrics"]["log_loss"]
            - nfl_only_oos_metrics["log_loss"],
        },
    }
    ...
    summary = {
        ...
        "cfb_excluded_seasons_note": CFB_EXCLUDED_SEASONS_NOTE,
        "cfb_games": len(cfb),
        "nfl_games": len(nfl),
        "features": FEATURES,
        "ridge": RIDGE,
        ...
        "target_b_base_plus_cfb_transfer_vs_base_v2": cell_plus_vs_base,
        "target_b_nfl_only_twin_vs_base": cell_nflonly_vs_base,
        "target_b_nfl_only_twin_vs_cfb_direct_v2": cell_nflonly_vs_cfbdirect,
        "base_four_term_loso_fold_betas": base_fold_betas,
        "base_four_term_in_sample_fit": base_in_sample,
    ...
    print("cfb_train_seasons", list(CFB_TRAIN_SEASONS))
    ...
    print(
        "cfb_direct_vs_base",
        json.dumps(cell_cfb_direct_vs_base["effect"]),
        cell_cfb_direct_vs_base["diff"],
    )
    print("plus_vs_base_v2", json.dumps(cell_plus_vs_base["effect"]), cell_plus_vs_base["diff"])
    print("plus_vs_base_v2_is_oos_gap", json.dumps(cell_plus_vs_base["in_sample_vs_oos_gap"]))
    print(
        "nflonly_vs_base", json.dumps(cell_nflonly_vs_base["effect"]), cell_nflonly_vs_base["diff"]
    )
    print(
        "nflonly_vs_cfbdirect_v2",
        json.dumps(cell_nflonly_vs_cfbdirect["effect"]),
        cell_nflonly_vs_cfbdirect["diff"],
    )
    print(
        "nflonly_v2_is_oos_gap",
        json.dumps(cell_nflonly_vs_cfbdirect["nfl_only_in_sample_vs_oos_gap"]),
    )
