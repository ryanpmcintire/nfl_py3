"""Graph `team_stat` off_sack_rate, stacked on PRODUCTION: does it beat what
is actually played, not a bare market baseline?

Predeclared in ``docs/graph_team_stat_on_production.md`` BEFORE this script
was pointed at an outcome column. Read that document first -- it declares the
candidate feature, the two arms, the rotation family/window, the null, the
...
* ``candidate`` -- ``weak_stack_graph_sack``: the SAME production feature set
  plus exactly one new column,
  ``graph_v2_team_stat_off_sack_rate_katz_diff``.
...
* ``screen``           -- the real look. Spends the family's assigned
  rotation window (``graph_off_sack_rate_on_production``); run once, after
...
from nfl_ats.graph_team_stat_production_feature import (  # noqa: E402
...
CANDIDATE_PROFILE: MarginFeatureProfile = "weak_stack_graph_sack"
REGRESSOR = "ridge"
RIDGE_ALPHA = 10.0

DEFAULT_FEATURES = REPO_ROOT / "data/processed/game_features_weak_stack_graph_sack.parquet"
...
def run_window(
    ...
    *,
    ...
    if leak_treatment:
        candidate_source[GRAPH_OFF_SACK_RATE_COLUMN] = pd.to_numeric(
    ...
    for (_season, _week), group in window.groupby(["season", "week"], sort=True):
        ...
        candidate_scoring = (
            ...
            if not leak_treatment
                ...
                **{GRAPH_OFF_SACK_RATE_COLUMN: pd.to_numeric(group["ats_margin"], errors="coerce")}
            )
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "the graph_off_sack_rate_on_production family's assigned [2014, 2016])",
    )
    ...
    configuration = {
        ...
        "predeclaration": "docs/graph_team_stat_on_production.md",
    ...
    output_dir = (
        REPO_ROOT
        / "artifacts"
        / (args.out or "graph_team_stat_off_sack_rate_on_production")
        / timestamp
    )
    write_experiment_artifact(
        ...
        "results.json",
        payload,
        command="graph-team-stat-off-sack-rate-on-production",
        metrics={"mode": args.mode, "status": result.get("status", "unknown")},
        notes=(
            "Graph ratings v2 team_stat off_sack_rate feature vs PRODUCTION weak_stack "
            "(not a bare market baseline); see docs/graph_team_stat_on_production.md."
        ),
    )
    ...
    if args.mode == "null" and result.get("status") == "scored":
        ...
        print()
        print(f"candidate (weak_stack_graph_sack) minus baseline (weak_stack), {args.mode}:")
        if pair is not None:
