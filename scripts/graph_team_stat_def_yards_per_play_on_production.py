"""Graph `team_stat` def_yards_per_play, stacked on PRODUCTION: does it beat
what is actually played, not a bare market baseline?

Predeclared in ``docs/graph_team_stat_def_ypp_on_production.md`` BEFORE this
script was pointed at an outcome column. Read that document first -- it
...
``scripts/graph_team_stat_off_sack_rate_on_production.py`` (the off_sack_rate
sibling) exactly, substituting the ``def_yards_per_play`` team_stat column --
the family that LEADS the 38-family screen by the conservative, null-adjusted
reference (docs/graph_ratings_v2_screen.md section 8, 95.5th percentile of its
own permutation null) rather than the naive zero-reference off_sack_rate led.
...
* ``candidate`` -- ``weak_stack_graph_def_ypp``: the SAME production feature
  set plus exactly one new column,
...
* ``screen``           -- the real look. Spends the family's assigned
  rotation window (``graph_def_ypp_on_production``); run once, after both
  checks above pass.
...
from nfl_ats.graph_team_stat_def_ypp_production_feature import (  # noqa: E402
...
CANDIDATE_PROFILE: MarginFeatureProfile = "weak_stack_graph_def_ypp"
REGRESSOR = "ridge"
RIDGE_ALPHA = 10.0

DEFAULT_FEATURES = REPO_ROOT / "data/processed/game_features_weak_stack_graph_def_ypp.parquet"
ROTATION_FAMILY = "graph_def_ypp_on_production"

# Rotation-assigned window (docs/graph_team_stat_def_ypp_on_production.md
# section 7); overridable via --seasons for the instrument checks, which run
# on the same window for comparability with the off_sack_rate sibling's own
# instrument checks.
...
def run_window(
    ...
    *,
    ...
    if leak_treatment:
        candidate_source[GRAPH_DEF_YPP_COLUMN] = pd.to_numeric(
    ...
    for (_season, _week), group in window.groupby(["season", "week"], sort=True):
        ...
        candidate_scoring = (
            ...
            if not leak_treatment
                ...
                **{GRAPH_DEF_YPP_COLUMN: pd.to_numeric(group["ats_margin"], errors="coerce")}
            )
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "the graph_def_ypp_on_production family's assigned window)",
    )
    ...
    configuration = {
        ...
        "predeclaration": "docs/graph_team_stat_def_ypp_on_production.md",
    ...
    output_dir = (
        REPO_ROOT
        / "artifacts"
        / (args.out or "graph_team_stat_def_yards_per_play_on_production")
        / timestamp
    )
    write_experiment_artifact(
        ...
        "results.json",
        payload,
        command="graph-team-stat-def-yards-per-play-on-production",
        metrics={"mode": args.mode, "status": result.get("status", "unknown")},
        notes=(
            "Graph ratings v2 team_stat def_yards_per_play feature vs PRODUCTION "
    ...
    if args.mode == "null" and result.get("status") == "scored":
        ...
        print()
        print(f"candidate (weak_stack_graph_def_ypp) minus baseline (weak_stack), {args.mode}:")
        if pair is not None:
