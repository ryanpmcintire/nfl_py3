"""Record the third-down reversion fade overlay's weekly arm to the challenger ledger.

Standalone entry point for
``nfl_ats.third_down_reversion_fade_overlay.record_third_down_reversion_fade_challenger_decisions``.
...
succeeds. The first real write is the 2026-09-08 Week 1 lock.

Usage (from the repo root, per AGENTS.md environment conventions)::

    .\\.tools\\uv.exe run --no-sync python scripts/record_third_down_reversion_fade_challenger.py
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = record_third_down_reversion_fade_challenger_decisions(
        args.artifacts_root, args.data_root
    )
