"""Record every total-respiratory-illness battery cell to
``registry/weak_signals.json``, reading ONLY from
``scripts/respiratory_battery_screen.py``'s results JSON -- no hand-typed
numbers.
...
landed with the expected classification/effect. Modeled directly on
``scripts/fluview_battery_record.py`` (same structure, reused not
reinvented).
"""
...
def _latest_results() -> Path:
    candidates = sorted((REPO / "artifacts" / "respiratory_battery").glob("*/results.json"))
    if not candidates:
        raise FileNotFoundError("no artifacts/respiratory_battery/*/results.json found")
    return candidates[-1]
...
def build_args(
    ...
    notes = (
        ...
        "Total-respiratory-illness home-market battery (docs/respiratory_battery.md); "
        "extends fluview_battery (state ILI, reliability 0.9814) from influenza alone to "
        "NSSP covid+flu+rsv ED-visit-%% summed AS-OF value; predeclared before any "
        "cover-rate sign was inspected. Point-in-time-safe as-of construction reuses "
        "fluview_battery_screen's checkpoint/merge_asof machinery verbatim, with issue "
        "epiweeks mapped to a conservative (never-early) release-date anchor since "
        "covidcast rows carry no native release_date field (docs/respiratory_battery.md "
        "section 3). NSSP state-level RAW coverage begins epiweek 202239 (Sept 2022), but "
        "the point-in-time-recoverable floor measured MUCH later: NSSP's 2022-2023 backfill "
        "was not published until 2024 (e.g. every CA 2022-season epiweek's earliest issue is "
        "202416, released 2024-04-20 -- measured, docs/respiratory_battery.md section 3 "
        "correction), so this cell's ACTUAL scored population is NFL seasons 2024-2025 only "
        "(coverage by season: 2022 0.0%, 2023 0.0%, 2024 97.0%, 2025 96.6%), narrower than "
        "fluview_battery's ~2017-18-2025 effective window."
    )
    args = [
        "weak-signals",
        "record",
...
def main() -> None:
    ...
    source = f"docs/respiratory_battery.md, {results_path.relative_to(REPO)}"
