"""POSITIVE CONTROL: does PERFECT weather knowledge move forced-pick accuracy?

Predeclaration: ``docs/weak_stack_v4.md``, "wind oracle". Written before this
script produced any output.

The forecast arm (``weak_stack_v4``) lost about a point at the opener. The
owner's hypothesis for why was forecast error. Measured, that is not the
explanation for TEMPERATURE -- the kickoff-nearest archive has a median 6.0
hour lead time and r = 0.964 against the observed value -- but WIND is
genuinely noisy at r = 0.649, and wind carries the most plausible football
mechanism.

This arm settles the whole channel instead of iterating on forecast sources.
It hands the model the weather that ACTUALLY happened -- a forecast of
infinite skill -- and asks whether accuracy moves at all.

* If the oracle does NOT beat the baseline, the weather channel is
  ``bounded_by_control``: no improvement in forecasting can recover an effect
  the oracle itself cannot produce. That is one of only two admissible closing
  grounds in AGENTS.md, and it closes the channel honestly rather than on an
  interval that happens to contain zero.
* If the oracle DOES beat it, the oracle-minus-forecast gap is exactly the
  headroom a better wind source could buy -- the number worth having BEFORE
  spending any effort on one.

The candidate table is deliberately LEAKY and must never be promoted or
published from. Both arms hold ridge_alpha, regressor and the paired opener
population identical to the v4 run, so the ONLY difference between the
forecast arm and this one is forecast skill.

Coverage caveat, stated up front: observed weather exists for 3,010 of 4,902
rows (61.4%) -- indoor games and unmapped stations have no observation. Nulls
are imputed on the training fold like every other family here, so this is an
oracle over the games that HAVE an observation, not over the whole slate.
"""
...
CANDIDATE_PROFILE: MarginFeatureProfile = "weak_stack_oracle_weather"
REGRESSOR = "ridge"
RIDGE_ALPHA = 10.0
...
V4_ALL_COLUMNS: tuple[str, ...] = OBSERVED_WEATHER_FEATURE_COLUMNS
...
def main() -> None:
    ...
    parser.add_argument(
        "--features",
        type=Path,
        default=REPO / "data/processed/game_features_weak_stack_oracle_weather.parquet",
    )
    ...
    if missing:
        raise SystemExit(
            ...
            "scripts/build_weak_stack_oracle_weather_table.py first."
        )
    ...
    if out_dir is None:
        ...
        out_dir = REPO / "artifacts" / "weak_stack_oracle_weather_eval" / ts
