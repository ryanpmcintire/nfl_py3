"""Build ``data/processed/game_features_weak_stack_graph_def_ypp.parquet``.
...
``def_yards_per_play`` column from
``nfl_ats.graph_team_stat_def_ypp_production_feature.attach_graph_def_ypp_feature``.
See ``docs/graph_team_stat_def_ypp_on_production.md``. Mirrors
``scripts/build_weak_stack_graph_sack_table.py`` (the off_sack_rate sibling).

Built on the production table on purpose, NOT on
``game_features_weak_stack_surface/_v3/_v4/_graph_sack.parquet``: the
question is whether the graph feature adds to PRODUCTION, and stacking it
...
Run:  .\\.tools\\uv.exe run --no-sync python scripts/build_weak_stack_graph_def_ypp_table.py
"""
...
DEST = REPO / "data/processed/game_features_weak_stack_graph_def_ypp.parquet"
...
def main() -> None:
    ...
    assert new_cols == [GRAPH_DEF_YPP_COLUMN], f"expected exactly one new column, got {new_cols}"
    ...
    coverage = widened[GRAPH_DEF_YPP_COLUMN].notna().mean()
