"""Record every FluView battery cell to ``registry/weak_signals.json``,
reading ONLY from ``scripts/fluview_battery_screen.py``'s results JSON --
...
def _latest_results() -> Path:
    candidates = sorted((REPO / "artifacts" / "fluview_battery").glob("*/results.json"))
    if not candidates:
        raise FileNotFoundError("no artifacts/fluview_battery/*/results.json found")
    return candidates[-1]
...
def build_args(
    ...
    notes = (
        ...
        "FluView home-market illness battery (docs/fluview_battery.md); predeclared "
        "before any cover-rate sign was inspected. Point-in-time-safe as-of "
        "construction (release_date-gated checkpoint table, docs/fluview_battery.md "
        "section 3); point-in-time-recoverable coverage measured to concentrate "
        "~2017-18 onward (state-level Epidata version history is effectively absent "
        "before that, measured this session), so this cell's effective scored window "
        "is narrower than the nominal 2010-2025 range despite the full ingest."
    )
    args = [
        "weak-signals",
        "record",
...
def main() -> None:
    ...
    source = f"docs/fluview_battery.md, {results_path.relative_to(REPO)}"
