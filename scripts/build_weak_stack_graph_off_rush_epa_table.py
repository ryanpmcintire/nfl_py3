"""Build ``data/processed/game_features_weak_stack_graph_off_rush_epa.parquet``.
...
``off_rush_epa_per_play`` column from
``nfl_ats.graph_team_stat_off_rush_epa_production_feature.attach_graph_off_rush_epa_feature``.
See ``docs/graph_team_stat_off_rush_epa_on_production.md``. Mirrors
``scripts/build_weak_stack_graph_def_ypp_table.py`` (the def_yards_per_play
sibling).

Built on the production table on purpose, NOT on
``game_features_weak_stack_surface/_v3/_v4/_graph_sack/_graph_def_ypp.parquet``:
the question is whether the graph feature adds to PRODUCTION, and stacking it
onto a profile already refused or still undecided at the opener would confound
the answer -- the same reason ``scripts/build_weak_stack_v4_table.py`` gives for
its own choice.

Never touches ``game_features_weak_stack.parquet`` or any other existing file.

Run:  .\\.tools\\uv.exe run --no-sync python scripts/build_weak_stack_graph_off_rush_epa_table.py
"""
...
DEST = REPO / "data/processed/game_features_weak_stack_graph_off_rush_epa.parquet"
...
def main() -> None:
    ...
    assert new_cols == [GRAPH_OFF_RUSH_EPA_COLUMN], (
    ...
    coverage = widened[GRAPH_OFF_RUSH_EPA_COLUMN].notna().mean()
