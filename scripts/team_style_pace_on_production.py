"""Team-style pace mismatch, stacked on PRODUCTION: does it beat what is
actually played, not a bare market baseline?

Predeclared in ``docs/team_style_pace_on_production.md`` BEFORE this script was
pointed at an outcome column. Read that document first -- it declares the
candidate column, the two arms, the rotation family/window, the expanding-
threshold deviation, the null, the positive control, and the recording rules.
Mirrors ``scripts/graph_team_stat_def_yards_per_play_on_production.py`` (the
single-arm original) exactly, substituting the team-style pace column.
...
* ``candidate`` -- ``weak_stack_team_style_pace``: the SAME production feature
  set plus exactly one new column, ``team_style_pace_mismatch_flag`` (the
  top-quartile absolute gap in prior-season league-centred seconds-per-play
  pace between the two teams; registry cell
  ``team_style_pace_mismatch_dog_cover``, reliability 0.489 -- the highest in
  the PBP-08 team-style battery).
...
measures the pace column's marginal contribution on top of everything the
production chain already explains, per the project's own "composition is not
the signal" lesson. Production carries no pace or play-tendency column at all
(**measured**: ``FEATURE_SETS["full_weak_stack"]`` is 90 columns, none of them
a pace column), and the flag is an ABSOLUTE gap, a shape a linear ridge cannot
form from its inputs even if both sides were present.
...
* ``screen``           -- the real look. Spends the family's assigned
  rotation window (``team_style_pace_on_production``); run once, after both
  checks above pass.
...
decision regardless of sign -- every family here is recorded
...
from nfl_ats.team_style_pace_production_feature import (  # noqa: E402
    TEAM_STYLE_PACE_MISMATCH_COLUMN,
)

BASELINE_PROFILE: MarginFeatureProfile = "weak_stack"
CANDIDATE_PROFILE: MarginFeatureProfile = "weak_stack_team_style_pace"
REGRESSOR = "ridge"
RIDGE_ALPHA = 10.0
...
def run_window(
    ...
    *,
    ...
    Probabilities rather than correctness are returned because the null
    check needs to re-grade the SAME fitted models against many permuted
    outcomes: model fitting never sees the grading outcome, so a permutation
    changes only the grade -- what makes a 200-permutation null affordable.
    ...
    effect the instrument must be able to see even inside the full
    ...
    if leak_treatment:
        candidate_source[TEAM_STYLE_PACE_MISMATCH_COLUMN] = pd.to_numeric(
    ...
    for (_season, _week), group in window.groupby(["season", "week"], sort=True):
        ...
        candidate_scoring = (
            ...
            if not leak_treatment
                ...
                **{
                    TEAM_STYLE_PACE_MISMATCH_COLUMN: pd.to_numeric(
...
def permuted_margins(
    ...
    ``docs/team_style_pace_on_production.md`` section 6 for why this null is
...
def flag_coverage(features: pd.DataFrame, seasons: tuple[int, ...]) -> dict[str, Any]:
    """Coverage and firing rate of the one new column INSIDE the window --
    reported with the result rather than assumed from the pooled table."""

    frame = regular_season_rows(features)
    frame = frame.loc[frame["result"].notna()]
    frame = frame.loc[frame["season"].astype(int).isin(seasons)]
    values = pd.to_numeric(frame[TEAM_STYLE_PACE_MISMATCH_COLUMN], errors="coerce")
    covered = values.notna()
    return {
        "n_window_games": len(frame),
        "coverage": float(covered.mean()) if len(frame) else float("nan"),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "the team_style_pace_on_production family's assigned window)",
    )
    ...
    coverage = flag_coverage(features, seasons)
    print(
        f"{TEAM_STYLE_PACE_MISMATCH_COLUMN} inside the window: "
        f"{coverage['n_window_games']} REG games, coverage {coverage['coverage']:.3%}, "
        f"fires on {coverage['firing_rate']:.3%} ({coverage['n_firing']} games)"
    )
    ...
    configuration = {
        ...
        "predeclaration": "docs/team_style_pace_on_production.md",
    ...
    output_dir = REPO_ROOT / "artifacts" / (args.out or "team_style_pace_on_production") / timestamp
    write_experiment_artifact(
        output_dir,
        "results.json",
        payload,
        command="team-style-pace-on-production",
        metrics={"mode": args.mode, "status": result.get("status", "unknown")},
        notes=(
            "Team-style pace-mismatch flag vs PRODUCTION weak_stack (not a bare market "
            "baseline); see docs/team_style_pace_on_production.md."
        ),
    )
    ...
    if args.mode == "null" and result.get("status") == "scored":
        ...
        print()
        print(f"candidate (weak_stack_team_style_pace) minus baseline (weak_stack), {args.mode}:")
        if pair is not None:
            low, high = pair["week_blocked_ci95"]
            season_low, season_high = pair["season_blocked_ci95"]
            print(
            ...
            print(
                ...
                f"{pair['candidate_accuracy'] * 100:.3f}%  season-blocked 95% "
                f"[{season_low * 100:+.3f}, {season_high * 100:+.3f}] P+ "
                f"{pair['season_blocked_probability_positive']:.3f}"
            )
        print(
            "home-pick rate: "
            + "  ".join(f"{arm} {rate:.3%}" for arm, rate in result["home_pick_rate"].items())
        )
        print(
