...
| UI-09 | 🚧 | Pool workbench | Rules, entries, confidence ranks, ownership scenarios. **Minimal version live 2026-08-25** (`docs/pool.html`, `src/nfl_ats/pool_workbench.py`): pool-rules input, forced-pick entry list, confidence ranks from the active forecast, ownership-scenario placeholder. Remaining: ownership scenarios beyond placeholder, entry persistence. |
