...
| PBP-07 | ✅ | Penalty state | **Closed on measurement 2026-08-17 without being built.** Team penalty rate is 0.0750 +/- 0.0101 with year-over-year reliability +0.261, and it is priced: the most-penalized quartile covers 49.85% against the least-penalized at 50.52%. Separately, the construct this row actually names is not measurable from local data -- neither play-by-play snapshot carries `penalty_type` or a description column (45 columns; only `penalty` and `penalty_yards`), so "pre-snap discipline" would need a wider nflverse column pull. Since total penalty discipline is already null against the market, that pull is not worth making |
