...
| UI-17 | ⬜ | Assistant refresh-diff answers | "What changed since Tuesday": feed the late-week refresh state (`pick_revisions` ledger rows + observed-movement flags for the current week) into the assistant corpus as one entry per refreshed game, with the frozen-Tuesday grade restated beside the refreshed lean. Done when a refreshed pick question answers from the refresh ledger (or truthfully reports no refresh yet) with both lines cited, pinned by fixture tests. No ledger writes; read-only like the rest of the panel. |
...
| LEAD-45 | 🔬 | CFB option-prep short rest | Defending the triple option on <10 days prep fails systematically. Predeclared direction: FADE option opponents on short rest. XLG-03 screen; prep-days observable from schedules. |
...
| LEAD-52 | ⬜ | Field-size observable capture | Record weekly entry counts/prize structure from public pool pages alongside the lock (the missing POL-05 input). No ATS direction (measurement infrastructure). Done when Week 1 observables land in a timestamped artifact with source hashes. |
...
| LEAD-59 | ⬜ | Historical officials archive | Per-game crew identity 2009+ from PFR referee logs (or the Wayback fallback) enabling LEAD-31/32/33 historically. Done when an immutable snapshot with provenance covers ≥90% of 2009–2025 REG games. |
| LEAD-60 | ⬜ | First-half line source probe | Assess whether the 2005–2016 VI backfill carries 1H lines; unblocks LEAD-02 if yes, kills it honest if no. Done when the probe reports coverage counts per season with parsed examples. |
