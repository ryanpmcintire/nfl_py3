...
| MKT-09 | ✅ | Provider licensing/quota audit | **Completed 2026-09-02** (`config/source_policies.json`, `src/nfl_ats/source_policy.py`, `docs/mkt09_licensing_audit.md`): the eight audited provider/source postures are now machine-readable and validated for annual review, acquisition, retention, redistribution, derived publishing, and Odds API quota constants. The audit’s RED NFL.com injury source still had four recurring jobs; all four are now explicitly PAUSED, and direct ingestion fails from tracked policy before any output or network call. Odds API live/backfill commands reject raw storage inside tracked/public repository paths while preserving configured private data roots, and quota guards are pinned to registry constants. No provider call or quota spend occurred. |
