...
| MKT-08 | 🔬 | Timing policy | Compare fixed weekly timestamps and news-triggered updates. **2026-09-02 audit foundation** (`scripts/timing_policy_audit.py`, `docs/timing_policy_audit.md`): the read-only audit found five retained fixed-checkpoint experiment artifacts and 11 clock-dispatched decision jobs, but zero prospective refresh-ledger rows and therefore zero structured news-trigger rows. It validates original decision ≤ source capture ≤ refresh < deadline/kickoff and inventories immutable capture timestamps without writing research artifacts. **2026-09-03 trigger schema landed:** the pick-revision ledger carries `trigger_type` (`clock_dispatch` default, `news_event` reserved), `trigger_source` (scheduler job id, via `--trigger-source`), and `trigger_observed_at_utc` (defaults to plan time), threaded through `record_plan`/`record_refresh`/`refresh-picks` CLI with legacy-row backfill and 2 focused tests. MKT-08 remains open until real post-lock refresh rows carry non-default triggers; no signal was scored or adjudicated. |
...
| MOD-11 | ✅ | Calibration suite | **Completed and reconciled 2026-09-02.** Platt, isotonic, and beta calibration are leak-safe and evaluated, and the formerly open calibration-by-regime pass was already executed by `scripts/calibration_by_regime_cfb_screen.py` with prediction-level artifacts under `artifacts/calibration_by_regime_cfb/20260818T214613Z`. The predeclared line-bucket arm reads -0.400 accuracy points, 95% [-1.190,+0.370], `probability_positive` 0.153 across 8,302 clean-core games; the season-era arm reads +0.157, [-0.330,+0.670], `probability_positive` 0.734 across 11,466 games, with calibration-gap sign agreement 0/2 on the two measurable era buckets. Both remain correctly recorded `unresolved_below_power` (`registry/weak_signals.json`); this closes the implementation/evaluation row without treating either result as a settled negative or changing the played card. Focused calibration and registry contracts: 14 passed on 2026-09-02. |
...
| LEAD-04 | 🔬 | Midweek steam detection | ≥3 books moving the same direction within one midweek hour = informed flow. Predeclared direction: FOLLOW the steam side at the frozen Tuesday line. Instrument: intraday_hourly 2023–2025 move events (the `book_leadership.md` event pipeline already parses them). |
...
| LEAD-15 | 🔬 | Backup tenure-gap valuation | The market applies a uniform backup haircut; backups with ≥2 years in the same system outperform fresh ones. Predeclared direction: BACK system-tenured backups, FADE new-system ones. Distinct covariate from the EPA-gap design (ledger row 23: tenure, not talent gap). |
| LEAD-16 | 🔬 | Midweek kicker change | Week-of kicker swaps inject PAT/FG variance the Tuesday line cannot contain. Predeclared direction: take the UNDERDOG (variance favors dogs), not a side-specific fade. Transactions + participation Snap data flag the population. |
| LEAD-17 | 🔬 | Specialist absence (snapper/punter) | Long-snapper/punter absence carries extreme per-play leverage (hidden yardage + PAT risk). Predeclared direction: FADE the affected team. Rare by construction — full 2009–24 depth, record regardless of width. |
...
| LEAD-33 | 🔬 | Late-season all-star crews | Hand-picked late-season crews call tighter, neutralizing physical underdogs. Predeclared direction: BACK favorites under all-star crews. Thin by construction; record regardless. **2026-09-05: source gap confirmed (read) -- `data/raw/officials/20260819T190537Z/officials.parquet`'s 9 columns (game_id, game_key, official_name, position, jersey_number, official_id, season, season_type, week) carry no all-star/Pro-Bowl/crew-designation marker, and no other locally snapshotted table does either. Per the task's own instruction, no proxy invented (a week-17/18-assignment heuristic was explicitly ruled out as invented, not measured). Stays 🔬, skipped this session. docs/officials_crew_leads.md.** |
...
| LEAD-38 | 🔬 | Snow-game home preparation | Equipment, routine, and footing familiarity in snow. Predeclared direction: BACK home teams in snow games. Rare; record regardless of width. **2026-09-05 source-gap, NOT screened** (`docs/weather_venue_leads.md`): measured — no local column names precipitation-TYPE (rain vs. snow) anywhere in `data/raw/*/schedules.parquet` or the validated forecast archive, and `forecast_precip_prob_pct` alone (a probability, no precip-type field) cannot distinguish a snow game from a rain game, so no proxy exists for "a snow game" specifically (unlike LEAD-37, which had a usable probability proxy). What would be needed: (a) an observed precipitation-TYPE join (a historical METAR/ASOS station archive keyed to stadium + kickoff hour), or (b) a temp-gated compound of the existing precip-probability archive with `forecast_temp_f`/`observed_temp_f` as a cruder "cold enough to be snow, not rain" proxy — not attempted this session since it was not predeclared before this measurement and the task instructed a source-gap note rather than inventing a new compound proxy for this specific lead. Stays 🔬, skipped this session. |
...
| LEAD-47 | 🔬 | CFB true-freshman road QB fade | Experience × hostile-environment interaction, steepest for true freshmen. Predeclared direction: FADE true-freshman road starters. XLG-03; distinct from XLG-06's recruiting-rating question (class × venue, not rating). |
...
| LEAD-49 | 🔬 | CFB portal-QB early starts | Transfer-portal star QBs are overpriced weeks 1–3 while timing and scheme settle. Predeclared direction: FADE portal-QB debuts early. XLG-03 + portal dates; never pool with the high-school recruiting entries (different uncertainty). |
...
| LEAD-53 | 🔬 | Sunday-AM Best Pick re-nomination | Re-nominate the weekly Best Pick on Sunday-morning refresh probabilities (POL-11 machinery): does late info improve the top-1 slot? Predeclared direction: refreshed nomination BEATS the Tuesday nomination. Prospective from Week 1, no window (paired within-week comparison). |
| LEAD-54 | 🔬 | Tiebreaker low-side shading | Public tiebreak entries cluster over the market total (public loves overs). Predeclared direction: shade the guess 1pt UNDER the market-implied median. Prospective record from Week 1; no window. |
| LEAD-55 | ⬜ | Beat-writer participation aggregator | Timestamped Wed/Thu beat-reported participation (limited/full/DNP) vs official designations; disagreements flag hidden status. No ATS direction until the feed exists (source build, then a reliability gate, then a family). Done when a 2026 prospective collection writes timestamped rows weekly. |
| LEAD-56 | ⬜ | Coach-speak phrase base rates | Mine presser language ("game-time decision", "hopeful", "we'll see") into absence base rates by phrase. Predeclared direction: vague phrases predict absence ABOVE the position baseline. Gated on a transcript-source assessment first; QBs only to start. |
