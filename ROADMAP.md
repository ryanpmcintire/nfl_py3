...
frozen active `weak_stack` model scores **52.83% against openers** on 1,537
paired 2020–2025 games, improving on the `player` baseline's 52.50% (vs
51.09% against closes on the same games; paired delta +1.35 points with
~99.9% probability positive; season-blocked interval vs the coin flip
excludes 50%). Closing lines, CLV, and vig are secondary.
...
1. **Read `docs/revisit_list.md` first.** A 2026-08-18 audit found four
   ...
   **The gating experiment is stated there and must run before any Tier 1
   ...
   suspect. If no, the list shrinks to the degenerate-bootstrap case. Also
   binding from that audit: reported intervals are 17-58% too narrow because
   the block bootstrap never refits (`docs/estimation_variance.md`), the
...
5. XLG-05 therefore has no cleared mechanism to transfer yet; it waits for
...
6. Score the active model and any frozen challengers on prospective 2026
   ...
   2026-09-08; no manual row insertion is possible, and `is_best_pick`
