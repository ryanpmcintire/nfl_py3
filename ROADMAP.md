...
| SKY-07 | 🚧 | Open benchmark suite | **Deterministic offline foundation implemented** (`src/nfl_ats/open_benchmark.py`, `docs/open_benchmark_suite.md`, 2026-09-02): canonical CSV/JSON exports pin schema, source URLs, SPDX license, hashes, row counts, chronological split boundaries, point-in-time timestamps, and withheld test labels. Submission exports are deterministic, complete, unscored, and pinned to the exact dataset manifest. SKY-07 remains open because no publishable dataset has passed the license/source/hosting gates and no public leaderboard host exists; no model comparison was run. |
